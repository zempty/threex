...
html5sticky.addNote = function() {
	...
	if (tnotes === max_notes) {
		alert('您添加的便笺已达上限' + max_notes + "，请删掉一些再添加。");
		return false;
	}
...
html5sticky.addUtilityButtons = function(el) {
	...
	$('.icons-footer')
			.append(
					'<img class="left shrink ficon" src="assets/img/shrink.png" alt="" title="shrink/expand">');
	$('.icons-footer')
			.append(
					'<img class="left alarm ficon" src="assets/img/alarm.png" alt="" title="set alarm for this sticky note">');
	$('.icons-footer')
			.append(
					'<img class="left lock ficon" src="assets/img/lock.png" alt="" title="lock this sticky note">');
	$('.icons-footer')
			.append(
					'<img class="left settings ficon" src="assets/img/settings.png" alt="" title="settings for note">');
	$('.icons-footer')
			.append(
					'<img class="left email ficon" src="assets/img/email.png" alt="" title="email this sticky note">');
	$('.icons-footer')
			.append(
					'<img class="left tweet ficon" src="assets/img/tweet.png" alt="" title="tweet this sticky note">');
	$('.icons-footer')
			.append(
					'<img class="left share ficon" src="assets/img/share.png" alt="" title="share this sticky note">');
...
html5sticky.getIdentifier = function(el) {
	...
	if (typeof identifier == 'undefined' || identifier == null) {
		identifier = $(el).closest('.note_common').find('[id^=idf_]')
				.attr('id');
	}
...
html5sticky.closeNote = function(el) {
	$(el).closest('.bignote')[html5sticky.getAnimation(true)]('slow',
			function() {
				$('#overlay').remove();
			});
};
...
html5sticky.getDateTime = function() {
	...
	return month + dsep + day + dsep + year + sep + hours + tsep + minutes
			+ sep + ampm;
...
html5sticky.editNote = function($clone, el) {
	...
	$clone.find('.pedit').val(ptext).css('margin-top', '5px').addClass('inset')
	...
	// put in Save button
	$('<a href="#" class="save_stickynote"><img src="assets/img/savebig.png" alt="" title="save note"></a>').css({
				position : 'absolute',
				top : 7,
				right : 60
			}).appendTo($clone);
	...
	// put in Tip
	$('<span class="tip">Tip: Allowed HTML tags: <code>').css({
		position : 'absolute',
		bottom : 10,
		left : 15
	}).appendTo($clone);
	...
	if (Modernizr.localstorage) {
		for (var i = 1; i <= max_notes; i++) {
			...
			if (localStorage.getItem(i + 'stickynote')) {
				...
				temp_array = localStorage.getItem(
						i + 'stickynote' + '|settings').split('|');
				bgcolor = temp_array[0];
				rotationClass = temp_array[1];

				// get position info
				temp_array = localStorage.getItem(i + 'stickynote' + '|pos')
						.split('|');
				pleft = temp_array[0];
				ptop = temp_array[1];

				// get text info
				temp_array = localStorage.getItem(i + 'stickynote' + '|text')
						.split('|');
				htext = temp_array[0];
				ptext = temp_array[1];
				...
				// append identifier
				$(stickynote).append(
						$('<span id="idf_' + i + 'stickynote' + '" />'));

				// $(stickynote).find('h2').attr('contentEditable', true);
				// $(stickynote).find('p').attr('contentEditable', true);

				$(stickynote).find('h2').text(
						html5sticky.stripTags(htext, allowed_tags));
				$(stickynote).find('p').html(
						html5sticky.stripTags(ptext, allowed_tags));
...
html5sticky.setup = function() {
	...
	if (typeof fontFamily != 'undefined' && fontFamily.indexOf('SimHei') >= 0) {
		$('.note_common ul').css({
			lineHeight : '10px',
			padding : '0px',
			margin : '0px'
